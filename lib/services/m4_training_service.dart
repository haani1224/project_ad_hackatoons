...
//   Future<int?> getTeacherIdFromAuth(String authUid) async {
...
//   Future<List<TrainingOption>> getAvailableTrainingOptions(int teacherId) async {
//     final applied = await _client
//       .from('trainings')
//       .select('training_option_id')
//       .eq('teacher_id', teacherId);
//     final appliedIds = (applied as List).map((e) => e['training_option_id'].toString()).toSet();
...
//         .where((o) => !appliedIds.contains(o.id.toString()))
//         .toList();
//   }
...
//       debugPrint('TEACHERS = $teacherRecords');
...
//         for (final o in optionDates as List) o['id'].toString(): o['training_date'] as String? ?? ''
//       };
...
//     final optionIds = (data as List).map((e) => e['training_option_id'].toString()).toList();
  ...
  Future<List<Map<String, dynamic>>> getTrainingOptionsWithApplicants() async {
    try {
      ...
      final applications = await _client
        .from('trainings')
        .select('id, teacher_uuid, training_option_id, status, reflection, certificate_url, photo_urls');
